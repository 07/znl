    ...
    try {
      ...
      switch (src.type) {
        ...
        case 'youtube_channel':
          ...
          break;
        case 'bilibili_user':
          items = await sources.bilibili_user(src);
          break;
        case 'bilibili_search':
          items = await sources.bilibili_search(src);
          break;
        case 'json':
        ...
        case 'fallback_videos':
          items = await fetchFallbackVideos(src.limit || n);
          break;
        default:
...
// 兑底视频:走 B 站搜索拿真实可播放的 BV 号
// 不再需要硬编码 BV 号,始终拿当下的热门
async function fetchFallbackVideos(n) {
  const queries = ['治愈', '助眠音乐', '人生低谷', '励志演讲'];
  const out = [];
  for (const q of queries) {
    if (out.length >= n) break;
    try {
      const items = await sources.bilibili_search({
        keyword: q,
        name: `B站·${q}`,
        order: 'totalrank',
        limit: 1
      });
      if (items && items[0]) out.push(items[0]);
    } catch (err) {
      console.warn(`[fallback_videos] search "${q}" failed: ${err.message}`);
    }
  }
  return out;
}
